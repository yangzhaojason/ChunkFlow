...
def train_loop(config: _config.TrainConfig):
    ...
    if config.pytorch_weight_path is not None:
        ...
        safetensors.torch.load_model(
            (model.module if isinstance(
                model, torch.nn.parallel.DistributedDataParallel) else model), model_path
        )
        logging.info(
    ...
    pbar = (
        tqdm.tqdm(total=config.num_train_steps, initial=global_step,
    ...
    while global_step < config.num_train_steps:
        ...
        for observation, actions in loader:
            ...
            if pbar is not None:
                ...
                pbar.set_postfix(
                    {"loss": f"{loss.item():.4f}",
                     "lr": f"{optim.param_groups[0]['lr']:.2e}", "step": global_step}
                )
