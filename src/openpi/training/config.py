...
CHUNKFLOW_REAL_JOINT_STAGE2_CHECKPOINT = os.environ.get(
...
class DataConfig:
    ...
    # Optional local root passed to LeRobotDataset and LeRobotDatasetMetadata.
...
class DataConfigFactory(abc.ABC):
    ...
    # Optional local root for a LeRobot dataset. Repo identity remains in repo_id.
    lerobot_root: str | None = None
...
class LeRobotLiberoDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransformOptional(
                    {
                ...
                _transforms.RepackTransform(
                    {
                        ...
                        "actions": "actions",
                        "prompt": "prompt",
                        # Pass-through RL signals for AWAC
                        "rewards": "rewards",
                        "discounts": "discounts",
                    }
                )
...
class RLDSTruthCartesianDataConfig(DataConfigFactory):
    ...
    def create(
        ...
        data_transforms = _transforms.Group(
            inputs=[
                truth_policy_cartesian.TruthInputsCartesian(
                    model_type=model_config.model_type,
                )
            ],
            outputs=[truth_policy_cartesian.TruthOutputsCartesian()],
        )
...
class RLDSTruthJointWithoutGripperDataConfig(DataConfigFactory):
    ...
    def create(
        ...
        data_transforms = _transforms.Group(
            inputs=[
                truth_policy_cartesian.TruthInputsJointWithoutGripper(
                    model_type=model_config.model_type,
                )
            ],
            outputs=[truth_policy_cartesian.TruthOutputsJointWithoutGripper()],
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
            lerobot_root=CHUNKFLOW_LIBERO_DATASET,
            base_config=DataConfig(prompt_from_task=True),
    ...
    TrainConfig(
        ...
        data=RLDSTruthCartesianDataConfig(
            repo_id="chunkflow_real_wood",
            # Set this to the path to your DROID RLDS dataset (the parent directory of the `droid` directory).
            rlds_data_dir=CHUNKFLOW_REAL_DATA_DIR,
    ...
    TrainConfig(
        ...
        data=RLDSTruthCartesianDataConfig(
            repo_id="chunkflow_real_cloth_v2",
            # Set this to the path to your DROID RLDS dataset (the parent directory of the `droid` directory).
            rlds_data_dir=CHUNKFLOW_REAL_DATA_DIR,
            action_space=truth_rlds_dataset.TruthActionSpace.CARTESIAN_POSITION,
            downsampled_and_repeated=False,
    ...
    TrainConfig(
        ...
        # We use RLDS data loading to make training on this large dataset tractable.
        # For fine-tuning on your own DROID dataset, see below.
        name="pi05_cytoderm11_joint_arm_move_chunkflow",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=16,
    ...
    TrainConfig(
        ...
        # We use RLDS data loading to make training on this large dataset tractable.
        # For fine-tuning on your own DROID dataset, see below.
        name="pi05_cytoderm13_joint_arm_move_chunkflow",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=16,
        ...
        data=RLDSTruthJointWithoutGripperDataConfig(
            repo_id="chunkflow_real_joint_v2",
            # Set this to the path to your DROID RLDS dataset (the parent directory of the `droid` directory).
            rlds_data_dir=CHUNKFLOW_REAL_DATA_DIR,
            action_space=truth_rlds_dataset.TruthActionSpace.JOINT_POSITION,
            downsampled_and_repeated=False,
    ...
    TrainConfig(
        ...
        data=RLDSDroidDataConfig(
            repo_id="droid_100",
            # Set this to the path to your DROID RLDS dataset (the parent directory of the `droid` directory).
            rlds_data_dir="datasets/droid_100_rlds",
            action_space=droid_rlds_dataset.DroidActionSpace.JOINT_POSITION,
        ),
        num_train_steps=10,
        batch_size=2,
