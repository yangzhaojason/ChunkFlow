    ...
    def __init__(
        ...
        *,  # Force keyword-only arguments
        ...
        def restructure(traj):
            ...
            # Rewards & discounts (safe defaults if absent)
            traj_len = tf.shape(traj["action"])[0]
            rewards = traj["reward"] if "reward" in traj else tf.zeros((traj_len,), dtype=tf.float32)
            if "discount" in traj:
                discounts = traj["discount"]
            else:
                is_term = traj["is_terminal"] if "is_terminal" in traj else tf.zeros((traj_len,), dtype=tf.bool)
                is_last = traj["is_last"] if "is_last" in traj else tf.zeros((traj_len,), dtype=tf.bool)
                discounts = 1.0 - tf.cast(tf.logical_or(is_term, is_last), tf.float32)
